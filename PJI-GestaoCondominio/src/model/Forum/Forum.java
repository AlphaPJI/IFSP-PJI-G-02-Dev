...
public class Forum {
	...
	public void todosComentarios(int codCategoria) {
		...
		int x = 0;
		for(ForumGS c : Comentarios.getTodosComentarios(codCategoria)) {
			x++;		
		}
		...
		int y = 0;
		for(ForumGS c : Comentarios.nomeBloco(idConta)) {
			y++;		
		}
	...
	public void comentariosUltimoMes(int codCategoria) {
		...
		int x = 0;
		for(ForumGS c : Comentarios.getComentariosMes(codCategoria)) {
			x++;		
		}
		if(x==0) {
	...
	public void comentariosUltimaSemana(int codCategoria) {
		...
		int x = 0;
		for(ForumGS c : Comentarios.getComentariosUltimaSemana(codCategoria)) {
			x++;		
		}
		if(x==0) {
			JOptionPane.showMessageDialog(null, "Não existem comentarios nesse periodo", "Alerta", JOptionPane.ERROR_MESSAGE);
